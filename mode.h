...
class ModeZigZagVert : public Mode {        
...
public:
    ...
    // Inherit constructor
    using Mode::Mode;
    Number mode_number() const override { return Number::VERTZIGZAG; }

    enum class VertDestination : uint8_t {
        vertA,  // Destination A
        vertB,  // Destination B
    };

    enum class VertDirection : uint8_t {
        UPWARD,
        DOWNWARD,
    } zigzag_vertdirection;
    ...
    void save_or_move_to_destination_vert(VertDestination ab_destvert);

    // return manual control to the pilot
    void return_to_manual_control_vert(bool maintain_target_vert);

    static const struct AP_Param::GroupInfo var_info[];
    float target_yaw_rgn = 0;

protected:

    const char *name() const override { return "VERTZIGZAG"; }
    const char *name4() const override { return "VERTZIGZ"; }
    uint32_t wp_distance() const override;
    int32_t wp_bearing() const override;
    float crosstrack_error() const override;
...
private:
    ...
    bool calculate_next_dest(VertDestination ab_dest, bool use_wpnav_alt, Vector3f& next_dest, bool& terrain_alt);
    void spray(bool b);
    bool calculate_side_dest(Vector3f& next_dest, bool& terrain_alt);
    bool calculate_wall_dist(Vector3f& next_dest, bool& terrain_alt);
    void move_to_side();
    void do_wall_correct();
   ...
   // AP_Int8  _rgn_quad;
    ...
    VertDestination ab_dest_stored;     // store the current destination
    ...
    //float current_dist_rgn1;
    //float current_dist_rgn2;
    float der_yaw_preverror = 0;
    float yaw_deg;
    float current_rgn1;
    float current_rgn2;
    float average_dist;
