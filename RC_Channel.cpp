...
void RC_Channel_Copter::init_aux_function(const aux_func_t ch_option, const AuxSwitchPos ch_flag)
{
    ...
    case AUX_FUNC::VERTZIG:
    case AUX_FUNC::VERTZIG_Auto:
    case AUX_FUNC::VERTZIG_SaveWP:
    case AUX_FUNC::ACRO:
...
bool RC_Channel_Copter::do_aux_function(const aux_func_t ch_option, const AuxSwitchPos ch_flag)
{
    switch(ch_option) {
        ...
        case AUX_FUNC::VERTZIG:
            ...
            do_aux_function_change_mode(Mode::Number::VERTZIGZAG, ch_flag);
#endif
            break;
...
#if MODE_ZIGZAGVERT_ENABLED == ENABLED
            if (copter.flightmode == &copter.mode_zigzagvert) {
                // initialize zigzag auto
                copter.mode_zigzagvert.init_auto();
                switch (ch_flag) {
                    case AuxSwitchPos::LOW:
                        copter.mode_zigzagvert.save_or_move_to_destination_vert(ModeZigZagVert::VertDestination::vertA);
                        break;
                    case AuxSwitchPos::MIDDLE:
                        copter.mode_zigzagvert.return_to_manual_control_vert(false);
                        break;
                    case AuxSwitchPos::HIGH:
                        copter.mode_zigzagvert.save_or_move_to_destination_vert(ModeZigZagVert::VertDestination::vertB);
                        break;
                }
            }
#endif
        ...
        case AUX_FUNC::VERTZIG_Auto:
            ...
            if (copter.flightmode == &copter.mode_zigzagvert) {
                switch (ch_flag) {
                case AuxSwitchPos::HIGH:
                    copter.mode_zigzagvert.run_auto();
                    break;
                default:
                    copter.mode_zigzagvert.suspend_auto();
                    break;
                }
            }
#endif
